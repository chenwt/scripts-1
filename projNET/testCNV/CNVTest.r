...
#     tFileMode <- "/Volumes/ys_lab_scratch/jh3283/net/AC13/BAF/chrCHR.freq"
#     nFileMode <- "/Volumes/ys_lab_scratch/jh3283/net/AC13/BAF/chrCHR.freq"
#     t.file <- gsub("CHR","10",)
#     n.file <- gsub("CHR","10","/Volumes/ys_lab_scratch/jh3283/net/AC19/BAF/chrCHR.freq")

#     t <- getSNP(t.file, snp.file)
#     n <- getSNP(n.file, snp.file)
#     # tAltFreq <- t$Altfreq[t$Altfreq>0]
#     nAltFreq <- n$Altfreq[n$Altfreq>0]
#     miu <- mean(nAltFreq)
#     delta <- var(nAltFreq)
...
#     yacas( "OldSolve({a /(a + b)==miu,a*b/((a+b)*(a+b+1))==delta},{a,b})")
