...
GetDBSNPs <- function(start, end){
	
	DBSNP.file.model <- Sys.getenv("DBSNPMODEL")
  	col.names <- c("CHROM", "CHROMSTART", "CHROMEND", "NAME", "REFUCSC", "CLASS", "AVHET")
  	
  	command <- past("cat ", DBSNP.file.model, " |awk '$2 >= ", format(start, scientific=FALSE),
  					" && $2 <= ", format(end, scientific=FALSE), " { print $0}'", " | cut -f1,2,3,4,5,6,7", sep="")
  	DBSNP.pipe <- pipe(command)
  	dbsnp.tmp <- readLines(DBSNP.pipe)
  	close(DBSNP.pipe)

  	if(!length(dnsnp.tmp)){
  		reads.df <- data.frame(CHROM=character(), CHROMSTART=integer(), CHROMEND=integer(), NAME=character(),
                           		REFUCSC=character(), CLASS=character(),AVHET=character())
  	}else {
  		    reads.df <- read.delim(file=textConnection(tmp), sep="\t",col.names=col.names, header=FALSE) 
   			reads.df <- reads.df[reads.df$CLASS=='single', ]
  	}
