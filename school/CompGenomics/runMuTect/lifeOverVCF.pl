#!/usr/bin/perl -w
# source: https://github.com/broadgsa/gatk/blob/master/public/perl/liftOverVCF.pl
# Runs the liftover tool on a VCF and properly handles the output
# Example: ./liftOverVCF.pl
...
my $cmd = "java -jar -Xmx1g $gatk/GenomeAnalysisTK.jar -T LiftoverVariants -R $oldRef.fasta -V:variant $in -o $unsorted_vcf -chain $chain -dict $newRef.dict";
...
$cmd = "java -jar -Xmx1g $gatk/GenomeAnalysisTK.jar -T FilterLiftedVariants -R $newRef.fasta -V:variant $sorted_vcf -o $out";
