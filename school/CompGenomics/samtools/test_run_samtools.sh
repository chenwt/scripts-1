...
qsub -N myjobCallY -e ./logs -o ./logs -b y -l mem=8G,time=2:: -S /bin/sh -cwd "bcftools view Y.var.bcf | vcfutils.pl varFilter -D 60 > Y.var.vcf"



##----------------filtering wiht DP4 and PV4##--

cat Y.var.vcf | grep -v "^#" | cut -f8 | sed 's/;/\t/g' | cut -f4 | sed -e 's/DP4=/\t/g'  | sed 's/\,/\t/g'| awk '{print $1+$2,'\t',$3+$4}' | head
