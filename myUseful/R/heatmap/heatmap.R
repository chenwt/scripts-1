...
rownames(leaders) <-leaders$Name
leaders <- leaders[,-1]
leadersMA <- data.matrix(leaders)
heatmap(leadersMA,Rowv=NA,Colv=NA,col=cm.colors(256),scale="column",margins=c(5,10),ColSideColors=rainbow(ncol(leadersMA),start=0,end=.3),RowSideColors=rainbow(nrow(leadersMA),start=0,end=.3))

savehistory("~/Dropbox/Viz/heatmap.R")
...
getColor <- function(val) {
    ...
    maxVal <- log(1065) 
    numCols <- 20
    pal <- colorRampPalette(c("#f4d5d0", "#692518"))  # Light red to dark red
    cols <- pal(numCols)
