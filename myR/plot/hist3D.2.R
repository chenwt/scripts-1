function (x = seq(0, 1, length.out = nrow(z)), y = seq(0, 1, 
    ...
    colkey = NULL, image = FALSE, contour = FALSE, panel.first = NULL, 
    ...
    ltheta = -135, lphi = 0, space = 0, add = FALSE, plot = TRUE) 
{
    ...
    plist <- initplist(add)
    dot <- splitdotpersp(list(...), bty, lighting, extendvec(x), 
        extendvec(y), z, plist = plist, shade, lphi, ltheta)
    ...
    if (ispresent(colvar)) {
        ...
        N <- length(col) - 1
        cmin <- clim[1]
        Cols <- matrix(nrow = nrow(z), col[1 + trunc((colvar - 
            cmin)/crange * N)])
    }
    else {
        iscolkey <- FALSE
    ...
    if (length(x) > 1) 
        xx <- extend(x)
    else xx <- dot$persp$xlim
    if (length(y) > 1) 
        yy <- extend(y)
    else yy <- dot$persp$ylim
    if (is.null(plist)) {
        do.call("perspbox", c(alist(range(xx), range(yy), dot$persp$zlim, 
            phi = phi, theta = theta, plot = plot, colkey = colkey, 
            col = col), dot$persp))
        plist <- getplist()
    }
    ...
    islight <- FALSE
    if (isshade) {
        to5facets <- function(val) as.vector(matrix(ncol = 5, 
            data = rep(val, length(ix)), byrow = TRUE))
        light <- setuplight(dot$shade$lphi, dot$shade$ltheta)
        norms <- matrix(nrow = 5, byrow = TRUE, data = c(0, -1, 
            0, 0, 1, 0, -1, 0, 0, 1, 0, 0, 0, 0, 1))
        if (dot$shade$type == "shade") {
            shade <- abs(dot$shade$shade)
            Sum <- 0.5 * (norms[, 1] * light[1] + norms[, 2] * 
                light[2] + norms[, 3] * light[3] + 1)
            Shade <- to5facets(Sum^shade)
        }
        else {
            isshade <- FALSE
            islight <- TRUE
            Normals <- list(u = to5facets(norms[, 1]), v = to5facets(norms[, 
                2]), w = to5facets(norms[, 3]))
        }
    }
    PolyX <- NULL
    PolyY <- NULL
    ...
    y.j <- yy[iy] + dy[iy]
    y.jp1 <- yy[iy + 1] - dy[iy]
    PolyX <- cbind(rbind(x.i, x.ip1, x.ip1, x.i), rbind(x.ip1, 
        x.i, x.i, x.ip1), rbind(x.i, x.i, x.i, x.i), rbind(x.ip1, 
        x.ip1, x.ip1, x.ip1), rbind(x.i, x.ip1, x.ip1, x.i))
    PolyY <- cbind(rbind(y.j, y.j, y.j, y.j), rbind(y.jp1, y.jp1, 
        y.jp1, y.jp1), rbind(y.j, y.jp1, y.jp1, y.j), rbind(y.j, 
        y.jp1, y.jp1, y.j), rbind(y.j, y.j, y.jp1, y.jp1))
    PolyZ <- cbind(rbind(z.k, z.k, z.kp1, z.kp1), rbind(z.k, 
        z.k, z.kp1, z.kp1), rbind(z.k, z.k, z.kp1, z.kp1), rbind(z.k, 
        z.k, z.kp1, z.kp1), rbind(z.kp1, z.kp1, z.kp1, z.kp1))
    COL <- rep(Col$facet, 5)
    BORD <- rep(Col$border, 5)
    if (isshade) {
        if (facets) {
        ...
        if (!is.na(border)) {
            ...
            BORD <- rgb(RGB)
            if (!is.null(dot$alpha)) 
                BORD <- setalpha(BORD, dot$alpha)
        }
    }
    else if (islight) {
        if (facets) 
            COL <- facetcols.shadelight(light, Normals, COL, 
                dot$shade)
        if (!is.na(border)) 
            BORD <- facetcols.shadelight(light, Normals, BORD, 
                dot$shade)
    }
    PolyX <- rbind(PolyX, NA)
    PolyY <- rbind(PolyY, NA)
    PolyZ <- rbind(PolyZ, NA)
    Proj <- project(colMeans(PolyX, na.rm = TRUE), colMeans(PolyY, 
        na.rm = TRUE), colMeans(PolyZ, na.rm = TRUE), plist, 
        TRUE)
    lwd <- dot$points$lwd
    if (is.null(lwd)) 
        lwd <- 1
    ...
    Poly <- list(x = PolyX, y = PolyY, z = PolyZ, col = COL, 
        border = BORD, lwd = rep(lwd, length.out = ncol(PolyX)), 
        lty = rep(lty, length.out = ncol(PolyX)), isimg = rep(0, 
            length.out = ncol(PolyX)), alpha = alpha, proj = Proj)
    class(Poly) <- "poly"
    if (image$add) 
    ...
    plist <- plot.struct.3D(plist, poly = Poly, segm = segm, 
        plot = plot)
    setplist(plist)
    invisible(plist$mat)
}