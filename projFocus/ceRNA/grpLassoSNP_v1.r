...
##------
dataExpSg   = sigmoid(normalize(dataExp))
...
regfit      = function(X, y, group, plotflag=0){
  ...
  if (setequal(colnames(X), names(group))){
       ...
       beta                = grpfit_selected$beta[-1]
        #residual_original   = sign(logit(y) - logit(toRange01(X %*% as.matrix(beta)))) * 
                              #((logit(y) - logit(toRange01(X %*% as.matrix(beta)))^2))
        residual_original   = logit(y) - X %*% as.matrix(beta)
                               #* ((logit(y) - X %*% as.matrix(beta))^2)
...
regfitPermu =   function(data_merge,group,nperm, plotflag=0) {
  ...
  while (count_perm < as.numeric(nperm) ) {
    y_permu             =   sigmoid(normalize(logit(y) + 
                            sample(fit_grpreg$residual_original,size=length(y),replace=TRUE)))
