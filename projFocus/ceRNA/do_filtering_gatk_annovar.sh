...
ExomeOrWGS=$2
	...
	grep PASS $i.germline-filtered.vcf.ann.vcf.tsv | grep HETEROZYGOUS |cut -f9 | sort  | uniq -c | sed 's/\s\+//' >> $i.summary

	##Annotate further the tsv with Drugbank + Clinical Trials
	# $PYTHON /ifs/scratch/c2b2/ngs_lab/sz2317/scripts/underWork/WGS/Mutable/mutableAddClinicalTrialsToTSV.py -c  /ifs/data/c2b2/ngs_lab/ngs/resources/mutable/ClinicalTrials.csv -d /ifs/data/c2b2/ngs_lab/ngs/resources/mutable/DrugNameIDSynonymsTargetgeneTargetSynonyms.csv -i $i.germline-filtered.vcf.ann.vcf.tsv > $i.germline-filtered.vcf.ann.vcf.tsv.clinicaltrial.tsv
	# if [[ $? != 0 ]]
	# then
...
# 	fi
	...
	# if [[ $ExomeOrWGS == "-E" ]]
	# then
	# 	cp  germline-filtered.vcf.ann.vcf.tsv.clinicaltrial.tsv $ResultDir/exonicvariants.ExomeAnalysis.tsv
	# 	cp  germline-filtered.vcf.ann.vcf.tsv.clinicaltrial.tsv.prioritised.tsv $ResultDir/exonicvariants.prioritised.ExomeAnalysis.tsv
	...
	# else
	# 	cp  germline-filtered.vcf.ann.vcf.tsv.clinicaltrial.tsv $ResultDir/exonicvariants.WGSAnalysis.tsv
	# 	cp  germline-filtered.vcf.ann.vcf.tsv.clinicaltrial.tsv.prioritised.tsv $ResultDir/exonicvariants.prioritised.WGSAnalysis.tsv
