...
try:
  ...
  print "ERROR:usage: python merge_snp_meth_cnvl3_v1.py -s <file: snp.mat> -m <file:meth.mat> -c <file:full path of cnv level3 files,one each line> -g <file: gene_exp.mat> "
  sys.exit()
for opt,arg in opts:
  if opt == '-h':
    print "Usage: python merge_snp_meth_cnvl3_v1.py -s <file: snp.mat> -m <file:meth.mat> -c <file:full path of cnv level3 files,one each line> -g <file: gene_exp.mat> "
    sys.exit()
  elif opt == '-c':
    inpc = arg
  elif opt == '-g':
...
# def inRegion(seg1,seg2):
#  """evaluation whether seg1 in in seg2"""
...
def isCNV(x):
  ''' evaluation whether a deletion is significant or not according to the cutoff'''
  x = float(x)
  if x > amp_cut or x < del_cut:
    return(1)
  else:
    return(0)
...
outf = open(out,'w')
outf.write("chr_pos_of_gene\tGenomicVariation\t" + "\t".join(fnArray ) + "\n")
...
with open(inps) as inpsf:
  for i,line in enumerate(inpsf):
	...
	if pts in chromDict[chrom].keys():
	    ...
	    #print "\t".join(map(str,chromDict[chrom][pts]))
        else:
